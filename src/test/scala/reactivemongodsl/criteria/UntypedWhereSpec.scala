 ...
 */
package reactivemongodsl.criteria
...
class UntypedWhereSpec
  ...
  "An Untyped where" should "support 1 placeholder" in {
    ...
    BSONDocument.pretty(q) shouldBe (
      BSONDocument.pretty(BSONDocument("a" -> BSONInteger(1)))
      )
  }
  ...
  it should "support 22 placeholders" in {
    ...
    BSONDocument.pretty(q) shouldBe (
      BSONDocument.pretty(
        BSONDocument(
          "$and" -> BSONArray(
            List.fill(22)(BSONDocument("p" -> BSONInteger(0)))
          )
        )
      )
      )
